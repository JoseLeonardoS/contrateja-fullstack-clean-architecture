...
public sealed class UserTests
{
    ...
    [Fact]
    public void ChangePhone_WhenSame_DoesNotUpdateUpdatedAt()
    {
        var user = CreateUser();
        var oldUpdatedAt = user.UpdatedAt;

        user.ChangePhone(user.Phone);
    ...
    [Fact]
    public void ChangeEmail_WhenSame_DoesNotUpdateUpdatedAt()
    {
        var user = CreateUser();
        var oldUpdatedAt = user.UpdatedAt;
    ...
    [Fact]
    public void ChangeAddress_WithNullCity_Throws()
    {
        var user = CreateUser();
        Assert.Throws<ArgumentNullException>(() => user.ChangeAddress(new State("SP"), null!, new Street("Av. Exemplo"), new ZipCode("01001000")));
    }

    [Fact]
    public void ChangeAddress_WithNullStreet_Throws()
    {
        var user = CreateUser();
        Assert.Throws<ArgumentNullException>(() => user.ChangeAddress(new State("SP"), new City("São Paulo"), null!, new ZipCode("01001000")));
    }

    [Fact]
    public void ChangeAddress_WithNullZipCode_Throws()
    {
        var user = CreateUser();
        Assert.Throws<ArgumentNullException>(() => user.ChangeAddress(new State("SP"), new City("São Paulo"), new Street("Av. Exemplo"), null!));
    }
    ...
    public void ChangeAddress_WhenDifferent_UpdatesAddressAndUpdatedAt()
    {
        ...
        Assert.Equal(newZipCode, user.ZipCode);
        Assert.True(user.UpdatedAt > oldUpdatedAt);
    }

    [Fact]
    public void ChangeAddress_WhenSame_DoesNotUpdateUpdatedAt()
    {
        var user = CreateUser();
        var oldUpdatedAt = user.UpdatedAt;

        user.ChangeAddress(user.State, user.City, user.Street, user.ZipCode);
    ...
    [Fact]
    public void ChangeAccountType_ToAllowed_DoesNotForceIsAvailableFalse()
    {
        var user = CreateUser(accountType: EAccountType.Freelancer, isAvailable: true);

        user.ChangeAccountType(EAccountType.Both);

        Assert.Equal(EAccountType.Both, user.AccountType);
    ...
    [Fact]
    public void ChangeAvailability_WhenSame_DoesNotUpdateUpdatedAt()
    {
        var user = CreateUser(isAvailable: false);
        var oldUpdatedAt = user.UpdatedAt;
