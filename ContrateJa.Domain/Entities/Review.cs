...
public sealed class Review : Entity
{
    ...
    {
        if (reviewerId <= 0)
            throw new ArgumentOutOfRangeException(nameof(reviewerId), "Reviewer Id cannot be lower than 1.");

        if (reviewedId <= 0)
            throw new ArgumentOutOfRangeException(nameof(reviewedId), "Reviewed Id cannot be lower than 1.");
